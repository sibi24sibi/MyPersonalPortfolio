
function CTA() {
  return (
    <div className="pt-36 relative w-full flex justify-center ">
      
      <div className="pt-14 pb-40">
        <h2 className="text-4xl md:text-5xl font-bold mb-10 text-[#00040f] dark:text-white">
          Interested in Working Together?
        </h2>
        <div className="flex justify-center">
        
function MyComponent({ condition }) {
  const dynamicStyle = {
    color: condition ? 'red' : 'blue',
    fontSize: condition ? '20px' : '16px',
    // Add more dynamic styles here as needed
  };
        <a
          href="mailto:sibi24sibi.2@gmail.com"
          className="cursor-pointer font-bold whitespace-nowrap 
        mt-6 px-8 py-3 dark:text-[#e1e1e1] border-2 rounded-full dynamic-border  bg-bg hover:bg-fun-pink hover:border-fun-pink transition-colors"
        >
          Get in Touch
        </a>
}
        </div>
      </div>

      
    </div>
  );
}

export default CTA;